// Your goal is to create a function to format a number given a template; if the 
// number is not present, use the digits 1234567890 to fill in the spaces.
...
function numericFormatter(template, number="123"){
    let ind = -1;

    let res = template.replace(/[a-z]/ig, (letter) => {
        ind = (ind + 1) == number.length ? 0 : (ind + 1);
        return number[ind]
    })
