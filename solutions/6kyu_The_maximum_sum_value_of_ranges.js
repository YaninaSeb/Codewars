...
function maxSum(arr,range){
    ...
    for (let i = 0; i < range.length; i++) {
        let startInd = range[i][0];
        let endInd = range[i].length > 1 ? range[i][1] : range[i][0]; 

        let sumSubArr = arr.slice(startInd, endInd + 1).reduce((acc, curr) => acc += curr, 0);

        arrSum.push(sumSubArr);
    }
