...
// where one or more letters at the end of one word will have the same letters (in the 
// same order) as the next word in the array. But, there are times when all the words 
// won't mesh.
...
// If all the words in the given array mesh together, then your code should return 
// the meshed letters in a string. You should return the longest common substring. 
// You won't know how many letters the meshed words have in common, but it will be 
// at least one.
...
function wordMesh(arr) {
    ...
    for (let i = 0; i < arr.length-1; i++) {
        ...
        for (let j = len; j >= 0; j--) {
            if (arr[i].slice(len - j) === arr[i+1].slice(0, j)) {
                curr = arr[i].slice(len - j);
                res += curr;
                break;
            }
        }
        if (curr == '') return 'failed to mesh';
        curr = '';
    }
    return res;
}
