...
// We want to code this string as a string of 0 and 1 and after 
// that be able to decode it.
// To code we decompose the given string in its decimal digits (in 
// the above example: 1 0 1 1 1 2 1 3) and we will code each digit.
...
// c) Write digit d as a binary string, with the rightmost bit 
...
// "11101111110110110111" resulting of concatenation of 11 10 11 
// 11 11 0110 11 0111.

// Task:
// - Given strng a string of digits representing a decimal number 
// the function code(strng) should return the coding of strng as 
// explained above.

// - Given a string strng resulting from the previous coding, decode 
// it to get the corresponding decimal string.
...
function code(strng) {
    ...
    for (let i = 0; i < strng.length; i++) {
        ...
        let countBits = binaryNum.toString().length;
        let num = '0'.repeat(countBits - 1) + '1' + binaryNum;
...
function decode(str) {
    ...
    while (str.length > 0) {
        let indNumOne = str.indexOf('1');
        let startInd = indNumOne + 1;
        let endInd = (indNumOne + 1) * 2
        let binaryNum = str.slice(startInd, endInd);
        let decNum = parseInt(binaryNum, 2);
        res += decNum;

        str = str.slice(endInd);
    }
