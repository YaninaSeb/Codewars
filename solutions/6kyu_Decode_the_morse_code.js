...
// message is written in Morse code, a single space is used to separate the character 
...
// - C++/Go/JavaScript/Julia/PHP/Python/Ruby/TypeScript: MORSE_CODE['.--']

// use MORSE_CODE[morse]
...
decodeMorse = function(morseCode){
    ...
    for (let i = 0; i < arr.length; i++) {
        let word = arr[i].split(' ').map((item) => MORSE_CODE[item]).join('');
        res.push(word);
    }
