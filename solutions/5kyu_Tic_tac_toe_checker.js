...
// the value is 0 if a spot is empty, 1 if it is an "X", or 2 if 
// it is an "O", like so:
...
function isSolved(board) {
  ...
  for (let i = 0; i < winnerParties.length; i++) {
    let party = winnerParties[i];
    if (party[0] === party[1] && party[0] === party[2] && party[0] !== 0) return party[0];
  };
